...
public class Line {
	...
	{
		return Math.sqrt((Math.pow((end2.getX()-end1.getX()), 2.0) + Math.pow((end2.getY()- end1.getY()), 2.0)));
		//You may find Math.pow(base, power) and Math.sqrt(num) useful
	}
	public Point midpoint()
	{
		double xPoint = ((end2.getX()+end1.getX())/2.0);
		double yPoint = ((end2.getX()+end1.getX())/2.0);
	...
	{
 		return Math.abs(otherLine.distance() - this.distance()) <= 0.001;
	}
